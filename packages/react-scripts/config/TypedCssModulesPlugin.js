...
class TypedCssModulesPlugin {
  constructor(options) {
    ...
    this.cssFilesPattern = path.join(paths.appSrc, '**/*.scss');
    ...
    return files
      .filter(file => {
        if (this.options.useCache === false || !this.cache.has(file)) {
          ...
          console.log(`[TypedCssModules] Skipped ${file}, using cache`);
          return false;
        }
      })
      .reduce((acc, file) => {
        return new DtsCreator(options)
          ...
          .then(content => {
            console.log('[TypedCssModules] Wrote ' + content.outputFilePath);
          ...
          .catch(reason => {
            Promise.reject(chalk.red('[TypedCssModules] Error ' + reason));
          });
      }, Promise.resolve([]));
  }
  ...
  apply(compiler) {
    ...
    compiler.plugin('before-run', (compilation, callback) => {
      glob(this.cssFilesPattern, null, (error, files) => {
        if (error) {
          callback('[TypedCssModules] Failed to search css glob files: ' + error);
          ...
          console.info('[TypedCssModules] No css module type definition file is generated');
          callback();
        }
      });
    });
